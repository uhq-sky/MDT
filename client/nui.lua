...
RegisterNUICallback("getLocales", function(data, cb)
    cb({
        ...
        data = {
            pageDescription = {
                none = 'Aucune description disponible.',
                homepage = 'Bienvenue sur le portail de gestion de la police. Accédez aux dossiers, avis de recherche et amendes.',
                records = 'Consultez et gérez les rapports d\'interventions et dossiers criminels.',
                wanteds = 'Liste des personnes recherchées et avis de recherche en cours.',
                fines = 'Gestion des amendes et contraventions.',
                department = 'Informations sur le département de police et le personnel.',
                liveMap = 'Carte en temps réel montrant la position des unités de police.',
            }
        }
    })
end)
...
RegisterNUICallback("getPlayerInfo", function(data, cb)
    ...
    cb({
        job = playerData.job and playerData.job.name or "",
        grade = playerData.job and (playerData.job.grade_label or playerData.job.grade_name or tostring(playerData.job.grade)) or ""
    })
end)
