...
Citizen.CreateThread(function()
    ExecuteSql("SELECT * FROM gfxmdt_fines", {}, function(result)
        ...
        for i = 1, #Fines do
            ...
            Fines[i].lastEdited = ('Last edited %s ago by %s'):format(GetTimeDiffAs(result[i].lastEdited.time),
                result[i].addedBy)
        end
        finesLoaded = true
    end)
                ...
                for k, v in pairs(Offenders[res[i].id].fines) do
                    ...
                    if fineData then
                        fine = {
                            ...
                            name = fineData.name ..
                            " - " .. fineData.jailTime ..
                            " " .. fineData.jailType .. " - " .. NumberToMoney(fineData.money),
                        }
                    else
                        fine = {
                            id = id,
    ...
    AddEventHandler("QBCore:Server:OnJobUpdate", function(source, job)
        ...
        if xPlayer then
            ...
            if job.name == "police" then
                ...
                Config.Department.totalPersonal = (Config.Department.totalPersonal):format(tonumber(string.sub(
                Config.Department.totalPersonal, 1, index - 1)) + 1)
