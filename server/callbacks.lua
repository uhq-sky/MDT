    ...
    ExecuteSql(("SELECT * FROM %s"):format(Config.UsersTable), {}, function(result)
        for i = 1, #result do
            ...
            if fw == "qb" or fw == "newqb" then
                local job = json.decode(result[i].job or "{}")
                local charinfo = json.decode(result[i].charinfo or "{}")
                local firstname = charinfo.firstname or "Inconnu"
                local lastname = charinfo.lastname or ""
                name = firstname .. " " .. lastname
                jobName = job.name
            elseif fw == "esx" or fw == "oldesx" then
                local firstname = result[i].firstname or "Inconnu"
                local lastname = result[i].lastname or ""
                name = firstname .. " " .. lastname
                jobName = result[i].job
            end
            ...
            table.insert(userList, {
                ...
                isPolice = jobName and Config.Jobs[jobName] ~= nil and Config.Jobs[jobName].ableToUse or false,
            })
        end
